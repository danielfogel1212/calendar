// Declare monthNames outside the event listener so it's globally accessible
const monthNames = [
...
async function updateCalendar(year, month) {
    ...
    console.log(birthdayData);  // Log the birthday data for debugging
    console.log(eventData);  // Log the event data for debugging
...
async function birthdays() {
    try {
        const response = await fetch('http://127.0.0.1:5000/birthdays');  // Fetch the birthdays data from the server
        const data = await response.json();  // Parse the response as JSON
        return data;  // Return the fetched data for further processing
    } catch (error) {
        console.error('Error fetching birthdays:', error);  // Handle any errors that occur during the fetch
    }
}

async function events() {
    try {
        const response = await fetch('http://127.0.0.1:5000/events');  // Fetch the events data from the server
        const data = await response.json();  // Parse the response as JSON
        return data;  // Return the fetched data for further processing
    } catch (error) {
        console.error('Error fetching events:', error);  // Handle any errors that occur during the fetch
    }
}
